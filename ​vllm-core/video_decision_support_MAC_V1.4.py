"""
MUDU-VLLM — Senaryo 3: Uctan uca video karar destek pipeline'i (7B) V1.5 — macOS
================================================================================
Katmanlar:
    [HAREKET]  YOLO + ByteTrack -> nesne takibi + sinif etiketleri + hareket anomalileri
    [KUANTUM]  Quantum inspired -> olasiliksal hafif hibrit risk fuzyonu
    [GORSEL ]  Qwen2.5-VL-7B    -> semantik tehdit yorumu + Turkce JSON  (Ollama)
    [SES    ]  Whisper          -> sesli ipuclari (imdat/help/ates)

** macOS uyumlu: Apple Silicon (M1/M2/M3) YOLO icin otomatik MPS (GPU) kullanir. **
** 7B icin Ollama'nin Mac surumu gerekir (asagidaki kurulum). Tamamen yerel/offline. **

--------------------------------------------------------------------------------
KURULUM (macOS):
    1) Ollama Mac surumu: https://ollama.com/download  (indir, kur)
    2) ollama pull qwen2.5vl:7b
    3) Modelfile (ayni klasorde, uzantisiz):
           FROM qwen2.5vl:7b
           PARAMETER num_ctx 16384
       ollama create qwen2.5vl-16k -f Modelfile
    4) brew install ffmpeg
    5) python3 -m venv venv && source venv/bin/activate
       pip install ultralytics opencv-python requests numpy faster-whisper torch torchvision

CALISTIRMA (video yolunu komut satirindan ver):
    source venv/bin/activate
    python3 video_decision_support_MAC_V1.5.py /Users/ad/Downloads/video.mp4

vLLM ile (final/GPU): MUDU_BASE_URL ve MUDU_MODEL env degiskenlerini ver.
--------------------------------------------------------------------------------
...
# --- macOS / capraz-platform cihaz secimi (YOLO icin) ---
def pick_yolo_device():
    """Apple Silicon -> 'mps', NVIDIA -> 0 (cuda), digerleri -> 'cpu'."""
    try:
        import torch
...
YOLO_DEVICE = pick_yolo_device()
print(f"YOLO cihazi: {YOLO_DEVICE}")
...
def run_yolo(video_path):
    ...
    results = YOLO(YOLO_WEIGHTS).track(source=video_path, persist=True, stream=True,
                                       ...
                                       vid_stride=VID_STRIDE, device=YOLO_DEVICE)
...
# [SES] Whisper (platform-bagimsiz gecici dosya)
...
def transcribe_audio_cues(video_path):
    try:
        ...
        return []
    wav = os.path.join(tempfile.gettempdir(), "mudu_audio.wav")
    try:
        subprocess.run(["ffmpeg", "-y", "-i", video_path, "-ac", "1", "-ar", "16000",
                        wav, "-loglevel", "error"], check=True)
    except (subprocess.CalledProcessError, FileNotFoundError):
        print("UYARI: ffmpeg yok -> ses atlandi. (brew install ffmpeg)")
        return []
...
def call_vl(vl_frames, cues, seen_classes):
    ...
    # num_ctx yalnizca Ollama'ya ozel; vLLM'de gonderme (baglam serve'de --max-model-len).
