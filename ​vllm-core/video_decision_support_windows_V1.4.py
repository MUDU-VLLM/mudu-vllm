"""
MUDU-VLLM — Senaryo 3: Uctan uca video karar destek pipeline'i (7B) V1.4 - WINDOWS
...
WINDOWS KURULUM:
    1) pip install ultralytics opencv-python requests numpy faster-whisper
    2) FFmpeg: https://www.gyan.dev/ffmpeg/builds/ (PATH'e ekle)
    3) Ollama: https://ollama.com/download/windows
    4) ollama pull qwen2.5vl:7b
    5) Modelfile: FROM qwen2.5vl:7b / PARAMETER num_ctx 16384
       ollama create qwen2.5vl-16k -f Modelfile

CALISTIRMA:  .\venv\Scripts\Activate.ps1 && python video_decision_support_V1.4.py
"""
...
if sys.platform == "win32":
    ...
    if not ffmpeg_found:
        print("UYARI: FFmpeg bulunamadı! Ses analizi çalışmayabilir.")
        print("FFmpeg'i şuradan indirin: https://www.gyan.dev/ffmpeg/builds/")
        print("ve PATH'e ekleyin veya C:\\ffmpeg\\bin klasörüne koyun.")

    # Windows için geçici dosya yolu
...
class AnomalyDetector:
    ...
    def check_proximity(self, detections, frame_idx):
        ...
        for v in vehicles:
            for u in vulns:
                ...
                if dist < 80:
                    self._add(ts, u.get("track_id", -1), u["cls"], 
    ...
    def _check(self, tid, frame_idx, ts):
        ...
        cls = hist[-1]["cls"]
        eff_fps = max(self.fps / max(VID_STRIDE, 1), 1)
        window = max(int(eff_fps * 2), 3)
        
        if len(hist) >= window and tid not in self._still_marked:
            last = hist[-window:]
            spread = np.std([h["cx"] for h in last]) + np.std([h["cy"] for h in last])
            if spread < 4 and cls in {"cat", "dog", "person"}:
                self._still_marked.add(tid)
                self._add(ts, tid, cls, "Hareketsiz nesne",
                         f"{cls} 2+ saniyedir hareketsiz (dusme/bilinc kaybi olasiligi)",
                         0.85, frame_idx)
...
def run_yolo(video_path):
    ...
    results = YOLO(YOLO_WEIGHTS).track(
        source=video_path, persist=True, stream=True,
    ...
    for step_idx, r in enumerate(results):
        ...
        if r.boxes is not None and r.boxes.id is not None:
            ...
            for (x1, y1, x2, y2), tid, c in zip(xyxy, ids, clss):
                ...
                dets.append({
                    "cls": name, "cx": float(cx), "cy": float(cy), 
                    "track_id": int(tid)
                })
...
def transcribe_audio_cues(video_path):
    ...
    wav = TEMP_DIR / "mudu_audio.wav"
    
    try:
        # Windows için ffmpeg komutu
        cmd = ["ffmpeg", "-y", "-i", video_path, "-ac", "1", "-ar", "16000", 
               str(wav), "-loglevel", "error"]
        subprocess.run(cmd, check=True, capture_output=True, text=True)
    except (subprocess.CalledProcessError, FileNotFoundError) as e:
        print(f"UYARI: ffmpeg hatasi -> ses atlandi. Hata: {e}")
        return []
    
    if not wav.exists():
        print("UYARI: Ses dosyasi olusturulamadi.")
        return []
    
    try:
        model = WhisperModel(WHISPER_SIZE, device="cpu", compute_type="int8")
        segments, _ = model.transcribe(str(wav), language="tr")
        cues = []
        for seg in segments:
            text = seg.text.strip()
        ...
        print(f"UYARI: Whisper hatasi: {e}")
        return []

# ==================== [KUANTUM] Quantum inspired ====================
class DecisionCore:
...
- HAYVANA ZARAR: arac veya kisi tarafindan bir hayvanin (kopek/kedi) ezilmesi,
  carpilmasi veya darp edilmesi (5199 sayili Hayvanlari Koruma Kanunu kapsaminda yasal ihlal)
...
2. Nesne dedektoru ipucu verir ama HATA yapabilir (dusuk cozunurlukte kopegi 'kisi'
   sanabilir). Karede NET bir kopek/kedi/hayvan goruyorsan, YOLO 'kisi' dese bile
   onu dogru tur olarak (kopek/kedi) yaz. Kendi gozlemin YOLO ipucundan onceliklidir.
...
8. RISK SEVIYESI kurali:
   - Kavga/arbede, saldiri/darp, hirsizlik, yerde hareketsiz/dusmus kisi -> risk EN AZ "Orta".
   - Arac ile yaya/hayvan yakinsamasi veya carpma riski (ipuclarinda "yakinsamasi" varsa) -> risk EN AZ "Orta".
   - Silah (tufek/tabanca/bicak), agir yaralanma, silahla saldiri, gerceklesmis carpma -> risk "Yuksek".
   - Sadece normal hareket/gecis, tehdit yok -> risk "Dusuk".
9. Bir hayvana arac carptiysa veya carpma riski varsa: risk EN AZ "Orta" olmali,
   olayi "olasi hayvan haklari ihlali" olarak not et ve actions'a
   "olayi kayit altina al, arac/plaka bilgisini not et" onerisini ekle.
10. Asagidaki ornek metni ASLA kopyalama.
...
if __name__ == "__main__":
    # Windows için video yolları
    videos = [
        r"C:\Users\saphi\Downloads\Arrest001_x264.mp4",
        # Daha fazla video ekleyebilirsiniz:
        # r"C:\Videos\örnek_video.mp4",
    ]
    
    # Varsayılan video yoksa kullanıcıdan iste
    if not videos or not os.path.exists(videos[0]):
        print("Video dosyası bulunamadı. Lütfen bir video yolu girin:")
        video_path = input("Video yolu: ").strip()
        if video_path:
            videos = [video_path]
        else:
            print("HATA: Video yolu girilmedi!")
            sys.exit(1)
    
    for path in videos:
        if not os.path.exists(path):
            print(f"UYARI: Video bulunamadı: {path}")
            continue
        ...
        try:
            ...
            print("HATA: Ollama'ya baglanilamadi.")
            print("Ollama'nın çalıştığından emin olun: http://localhost:11434")
