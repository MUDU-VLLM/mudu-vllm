"""
MUDU-VLLM — Senaryo 3: Uctan uca video karar destek pipeline'i (7B)
====================================================================
Katmanlar:
...
ÖN KOSULLAR (bir kerelik, takim arkadaslari icin):
    1) ollama pull qwen2.5vl:7b
    2) Modelfile olustur:   FROM qwen2.5vl:7b
...
CALISTIRMA:  source venv/bin/activate && python3 video_decision_support_V1.2.py
"""
...
# YAPILANDIRMA
# ----------------------------------------------------------------------------
BASE_URL = "http://localhost:11434/v1"   # vLLM final: "http://localhost:8000/v1"
MODEL    = "qwen2.5vl-16k"               # 16384 baglamli surum; yoksa "qwen2.5vl:7b"
                                         # vLLM final: "Qwen/Qwen2.5-VL-7B-Instruct"

YOLO_WEIGHTS   = "yolov8m.pt"   # n=hizli/zayif, m=dengeli, x=yavas/guclu
MAX_VL_FRAMES  = 8
SEND_WIDTH     = 768
KEEP_EVERY_SEC = 0.4
NUM_CTX        = 16384
VID_STRIDE     = 6              # YOLO her N karede bir tarar -> ~Nx hizli (CPU icin sart)
ENABLE_AUDIO   = True
WHISPER_SIZE   = "small"
...
def run_yolo(video_path):
    ...
    for step_idx, r in enumerate(results):
        frame_idx = step_idx * VID_STRIDE   # gercek kare -> zaman damgasi dogru
        dets = []
...
def analyze(video_path):
    ...
    if YOLO_OK:
        ...
        try:
            ...
            # Sessizce atlamasin: gercek hatayi yaz, sonra orneklemeye dus
